...
class PrintRentAgreement(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not rent_agreement_id:
            ...
            p.drawString(50, 720, 'License No: ')
            p.drawString(260, 720, 'Issued By: ')
            p.drawString(50, 670, 'Date Issued: ')
            p.drawString(260, 670, 'Expiry Date:')
            p.drawString(510, 880, 'Home Address:')
            p.drawString(510, 830, 'Tel No.:')
            p.drawString(760, 830, 'Client Identity:')
            p.drawString(50, 520, 'Total Amount:')

            p.drawString(560,730, 'Driver Name')
            p.drawString(770,730, 'License No')
            p.drawString(560, 680, 'Passport No')
            p.drawString(770, 680, 'Nationality')
            ...
            p.drawString(130, 720, rent_agreement.client.license_no if rent_agreement.client else '')
            p.drawString(330, 720, rent_agreement.client.issued_by if rent_agreement.client else '')
            p.drawString(150, 670, rent_agreement.client.date_of_issue.strftime('%d/%m/%Y') if rent_agreement.client else '')
            p.drawString(350, 670, rent_agreement.client.expiry_license_date.strftime('%d/%m/%Y') if rent_agreement.client else '')
            ...
            p.drawString(580,710, rent_agreement.driver.driver_name)
            p.drawString(790,710, rent_agreement.driver.driver_nationality)
            p.drawString(580, 660, rent_agreement.driver.driver_passport_no)
            p.drawString(790, 660, rent_agreement.driver.driver_nationality)
